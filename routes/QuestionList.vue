<template>
  <div class="question-management">
    ...
    <!-- 列表视图 -->
    <div v-if="!isCategoryView" class="table-container">
      <el-table
        v-loading="loading"
          ...
          <template #default="{ row }">
            <div class="row-operations">
              <el-button
                ...
                @click.stop="editQuestion(row)"
              >
                编辑
              </el-button>
              <el-button type="danger" size="small" @click.stop="deleteQuestion(row.id)">
                删除
              </el-button>
            </div>
          </template>
    ...
    <div v-else class="category-view">
      ...
      >
        <el-select
          v-model="selectedCategorySubjectId"
          placeholder="选择学科"
          style="width: 150px"
          @change="handleCategorySubjectChange"
        >
        ...
        <el-select
          v-model="selectedCategorySubcategoryId"
          placeholder="选择题库"
          style="width: 150px"
          @change="handleCategorySubcategoryChange"
        >
      ...
      <div
        v-if="selectedCategorySubjectId && selectedCategorySubcategoryId"
        class="category-questions-table"
      >
        <el-table
          v-loading="loading"
...
const categoryQuestions = computed(() => {
  ...
  const filteredQuestions = props.questions.filter(q => {
    ...
    return (
      String(questionSubjectId) === String(selectedCategorySubjectId.value) &&
...
const refreshQuestions = async () => {
  try {
    ...
    if (filterSubjectId.value) {
      // 如果选择了学科，加载该学科的所有题目
      await questionStore.loadQuestions(parseInt(filterSubjectId.value), null, true)
    } else {
      // 如果没有选择学科，加载所有题目
