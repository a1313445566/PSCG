  ...
  try {
    ...
    if (data.subcategories && data.subcategories.length > 0) {
      for (const subcategory of data.subcategories) {
        await db.run('INSERT INTO subcategories (id, subject_id, name, icon_index) VALUES (?, ?, ?, ?)', 
    ...
    if (data.users && data.users.length > 0) {
      for (const user of data.users) {
        await db.run('INSERT INTO users (id, student_id, name, grade, class) VALUES (?, ?, ?, ?, ?)', 
    ...
    if (data.questions && data.questions.length > 0) {
      for (const question of data.questions) {
        ...
        await db.run('INSERT INTO questions (id, subject_id, subcategory_id, content, type, options, correct_answer, explanation, audio_url, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    ...
    if (data.answer_records && data.answer_records.length > 0) {
      for (const record of data.answer_records) {
        await db.run('INSERT INTO answer_records (id, user_id, subject_id, subcategory_id, total_questions, correct_count, time_spent, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', 
    ...
    if (data.question_attempts && data.question_attempts.length > 0) {
      for (const attempt of data.question_attempts) {
        await db.run('INSERT INTO question_attempts (id, user_id, question_id, subject_id, subcategory_id, user_answer, is_correct, answer_record_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
...
router.post('/import-local-data', async (req, res) => {
  try {
    ...
    if (data.subjects && data.subjects.length > 0) {
      for (const subject of data.subjects) {
        await db.run('INSERT INTO subjects (name, icon_index) VALUES (?, ?)', 
          [subject.name, subject.iconIndex || 0]);
      }
    }
    ...
    if (data.subcategories && data.subcategories.length > 0) {
      for (const subcategory of data.subcategories) {
        await db.run('INSERT INTO subcategories (subject_id, name, icon_index) VALUES (?, ?, ?)', 
          [subcategory.subjectId, subcategory.name, subcategory.iconIndex || 0]);
      }
    }
    ...
    if (data.users && data.users.length > 0) {
      for (const user of data.users) {
        await db.run('INSERT INTO users (student_id, name, grade, class) VALUES (?, ?, ?, ?)', 
          [user.studentId, user.name, user.grade, user.class]);
      }
    }
    ...
    if (data.questions && data.questions.length > 0) {
      for (const question of data.questions) {
        await db.run('INSERT INTO questions (subject_id, subcategory_id, content, type, options, correct_answer, explanation, audio_url, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
          [question.subjectId, question.subcategoryId, question.content, question.type, question.options, question.correctAnswer, question.explanation, question.audioUrl, question.imageUrl]);
      }
    }
