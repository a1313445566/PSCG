...
router.get(
  '/navigation-menus',
  adminAuth,
  checkPermission('basic-settings', 'view'),
  async (req, res) => {
    try {
...
router.get('/navigation-menus/visible', async (req, res) => {
  try {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const result = await navigationService.updateSortOrder(menus)
