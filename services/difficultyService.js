...
class DifficultyService {
  ...
  async adjustQuestionDifficulty(questionId) {
    try {
      ...
      const { total_attempts, correct_count, weighted_attempts, weighted_correct_count } = attempts[0];
      
      // 计算加权错误率
      const weightedErrorRate = (weighted_attempts - weighted_correct_count) / weighted_attempts;
      
      // 获取当前难度和题目类型
      const question = await db.get('SELECT difficulty, type FROM questions WHERE id = ?', [questionId]);
      if (!question) {
      ...
      // 根据错误率调整难度（使用更细致的阈值）
      if (total_attempts >= 10) { // 至少10次尝试才调整难度
        // 根据题目类型调整阈值
  ...
  async adjustSubcategoryDifficulty(subcategoryId) {
    try {
      // 获取题库信息
      const subcategory = await db.get('SELECT id, name FROM subcategories WHERE id = ?', [subcategoryId]);
      if (!subcategory) {
        return { success: false, message: '题库不存在' };
      }
