...
请返回 JSON 格式的查询计划（只返回 JSON）：
{
  ...
  async analyzeWithResults(question, queryResults) {
    const prompt = `你是一个专业的教育数据分析师，拥有完全自主权。
...
3. 语言风格（专业、通俗、幽默等）
4. 是否需要图表
5. 图表的所有细节（类型、样式、颜色、标签等）
6. 如果学生不存在，如何提示用户（你可以自主决定语气和内容）

**图表配置**：
如果你想展示图表，在回答末尾提供完整的 VChart 配置：
\`\`\`vchart
{
...
**重要规则**：
1. 配置必须是纯JSON格式，不能包含JavaScript函数或箭头函数
2. data字段格式：[{ "id": "data", "values": [{ "field1": "value1", "field2": "value2 }, ...] }]
3. xField和yField对应values数组中对象的字段名
4. 支持的配置项：type, data, xField, yField, title, axes, legends, color, label, tooltip
5. 不要使用format、formatter等需要函数的配置项
...
你可以自由决定分析角度、输出格式、是否需要图表。如果想展示图表，在末尾提供 VChart 配置。`;
...
${imageUrls.length > 0 ? `注意：部分题目选项包含图片，请结合图片内容分析学生可能的错误原因。` : ''}

请从以下角度分析：
1. 错题的共同特点（知识点、题型、难度等）
2. 可能的错误原因（概念不清、审题失误、计算错误等）
3. 针对性的学习建议
4. 如果选项包含图片，请分析图片内容对答题的影响
