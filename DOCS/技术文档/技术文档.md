...
- **排行榜系统**：基于积分和正确率的排名
- **后台管理**：题目管理、用户管理、数据备份
- **AI 功能**：AI 自然语言分析、AI 数据增强、AI 学习风格分析
...
| VisActor | 2.0.20 | 数据可视化（VChart + VMind） |
| Vite | 5.x | 构建工具 |
| Quill | 2.0.3 | 富文本编辑器 |
...
├── utils/              # 工具函数
│   ├── api.js          # API 封装（重试、防抖、超时）
│   ├── apiCache.js     # API 缓存
│   ├── cacheConfig.js  # 缓存配置
│   ├── chartGenerator.js # 图表配置生成（VChart）
│   ├── csrf.js         # CSRF Token 管理
│   ├── database.js     # 前端数据库配置
│   ├── dateUtils.js    # 日期处理
│   ├── draftStorage.js # 草稿存储
│   ├── errorCollectionUtils.js # 错题收集工具
│   ├── format.js       # 格式化工具（百分比、数字、时间）
│   ├── imageUpload.js  # 图片上传和队列管理
│   ├── lazyLoad.js     # 懒加载工具
│   ├── markdown.js     # Markdown 渲染
│   ├── message.js      # 消息提示封装
│   ├── shuffleOptions.js # 选项打乱
│   └── xss-filter.js   # XSS 防护
├── styles/             # 样式
│   ├── global.css      # 全局样式
│   ├── loading.css     # 加载动画样式
...
│   └── users.js        # 用户管理路由
├── services/           # 服务
│   ├── aiService.js    # AI 服务（豆包 AI 集成）
│   ├── aiQueueService.js # AI 任务队列服务
│   ├── cache.js        # 缓存服务（memory-cache）
│   ├── database.js     # 数据库连接池服务
│   ├── difficultyService.js # 智能难度调整服务
│   ├── fileReferenceService.js # 文件引用管理服务
│   ├── passwordHash.js # 密码哈希服务
│   └── validationService.js # 输入验证服务
├── middleware/         # 中间件
│   ├── adminAuth.js    # 管理员认证（JWT Token）
│   ├── aiInputValidation.js # AI 输入验证（参数校验）
│   ├── csrf.js         # CSRF 防护
│   ├── dbPerformance.js # 数据库性能监控
│   ├── rateLimit.js    # IP/用户限流（防刷分）
│   ├── responseTime.js # 响应时间监控
│   └── signatureCache.js # 签名缓存（防重放攻击）
├── server.cjs          # 后端服务器
└── package.json        # 依赖配置
```
...
**功能**：
- AI 自然语言分析：分析学生答题行为和学习风格
- AI 数据增强：自动生成题目和解析
- AI 学习风格分析：基于答题数据识别学习风格
...
**关键代码**：
- `routes/ai.js`：AI 相关API
- `src/components/admin/ai/`：AI 分析组件
...
| 端点 | 方法 | 功能 |
|------|------|------|
| `/api/ai/analyze` | POST | AI 自然语言分析 |
| `/api/ai/learning-style` | POST | 学习风格分析 |
| `/api/ai/data-enhancement` | POST | AI 数据增强 |
| `/api/ai/history` | GET | 获取 AI 分析历史 |
