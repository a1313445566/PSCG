    ...
    <el-form :model="form" label-width="100px" @submit.prevent="handleSubmit">
      <el-form-item
        ...
        :rules="[
          ...
          { pattern: /^\d{2}$/, message: '学号必须为2位数字', trigger: 'blur' }
        ]"
      >
        <el-input
          ...
          placeholder="请输入2位学号"
          maxlength="2"
          @input="form.student_id = form.student_id.replace(/[^0-9]/g, '').slice(0, 2)"
        ></el-input>
      </el-form-item>
      ...
      >
        <el-select v-model="form.class" placeholder="请选择班级">
          <el-option
            ...
            :key="classNum.id || classNum"
            :label="classNum.name || classNum"
            :value="classNum.name || classNum"
          ></el-option>
        </el-select>
      </el-form-item>
    </el-form>
