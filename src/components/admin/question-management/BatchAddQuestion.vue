...
const parseBatchQuestions = () => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numberedQuestionMatch) {
      ...
      // 保留括号但移除括号中的答案
      const contentWithEmptyBrackets = questionText + numberedQuestionMatch[3].replace(/[A-Da-d]+/, '') + postfix;
      currentQuestion = {
      ...
      // 保留括号但移除括号中的答案
      const contentWithEmptyBrackets = questionText + unnumberedQuestionMatch[2].replace(/[A-Da-d]+/, '') + postfix;
      currentQuestion = {
        content: contentWithEmptyBrackets,
        answer: answer,
        type: questionType,
