...
-- 检查并添加 final_answer 列
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND COLUMN_NAME = 'final_answer');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE answer_behavior ADD COLUMN final_answer VARCHAR(50) COMMENT ''最终答案''', 
  'SELECT ''Column final_answer already exists''');
...
-- 检查并添加 is_correct 列
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND COLUMN_NAME = 'is_correct');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE answer_behavior ADD COLUMN is_correct TINYINT(1) COMMENT ''最终是否正确''', 
  'SELECT ''Column is_correct already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 检查并添加 hesitation_time 列
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND COLUMN_NAME = 'hesitation_time');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE answer_behavior ADD COLUMN hesitation_time INT DEFAULT 0 COMMENT ''犹豫时间（秒）''', 
  'SELECT ''Column hesitation_time already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 检查并添加 skipped_and_returned 列
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND COLUMN_NAME = 'skipped_and_returned');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE answer_behavior ADD COLUMN skipped_and_returned TINYINT(1) DEFAULT 0 COMMENT ''是否跳过后返回''', 
  'SELECT ''Column skipped_and_returned already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 检查并添加 session_id 列
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND COLUMN_NAME = 'session_id');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE answer_behavior ADD COLUMN session_id VARCHAR(100) COMMENT ''答题会话ID''', 
  'SELECT ''Column session_id already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 检查并添加 selected_answer 列（修复缺失字段）
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND COLUMN_NAME = 'selected_answer');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE answer_behavior ADD COLUMN selected_answer VARCHAR(50) COMMENT ''选中的答案''', 
  'SELECT ''Column selected_answer already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @index_exists = (SELECT COUNT(*) FROM information_schema.STATISTICS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'answer_behavior' AND INDEX_NAME = 'idx_session');
SET @sql = IF(@index_exists = 0, 
  'ALTER TABLE answer_behavior ADD INDEX idx_session (session_id)', 
  'SELECT ''Index idx_session already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'learning_progress' AND COLUMN_NAME = 'ai_suggestion');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE learning_progress ADD COLUMN ai_suggestion TEXT COMMENT ''AI 生成的学习建议''', 
  'SELECT ''Column ai_suggestion already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'learning_progress' AND COLUMN_NAME = 'last_suggestion_at');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE learning_progress ADD COLUMN last_suggestion_at DATETIME COMMENT ''上次建议时间''', 
  'SELECT ''Column last_suggestion_at already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'learning_progress' AND COLUMN_NAME = 'suggestion_type');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE learning_progress ADD COLUMN suggestion_type VARCHAR(50) COMMENT ''建议类型：review/practice/challenge''', 
  'SELECT ''Column suggestion_type already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'learning_progress' AND COLUMN_NAME = 'recent_accuracy');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE learning_progress ADD COLUMN recent_accuracy FLOAT DEFAULT 0 COMMENT ''最近5次正确率''', 
  'SELECT ''Column recent_accuracy already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'learning_progress' AND COLUMN_NAME = 'accuracy_trend');
SET @sql = IF(@col_exists = 0, 
  'ALTER TABLE learning_progress ADD COLUMN accuracy_trend VARCHAR(10) COMMENT ''趋势：improving/declining/stable''', 
  'SELECT ''Column accuracy_trend already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 9. 重构 ai_analysis_cache 表
-- 检查表是否存在，如果不存在则创建新表
CREATE TABLE IF NOT EXISTS ai_analysis_cache (
  id INT AUTO_INCREMENT PRIMARY KEY,
  cache_key VARCHAR(255) COMMENT '缓存键',
  query_hash VARCHAR(64) COMMENT '查询哈希',
  query_text TEXT COMMENT '查询原文',
  cache_data TEXT COMMENT '缓存数据（JSON）',
  result_text TEXT COMMENT 'AI分析结果',
