...
async function main() {
  ...
  try {
    ...
    // 查询现有管理员
    const admins = await db.all('SELECT id, username, created_at FROM admin_credentials ORDER BY id');
    ...
    if (admins.length === 0) {
      ...
      admins.forEach((admin, index) => {
        console.log(`   ${index + 1}. ${admin.username} (创建于: ${new Date(admin.created_at).toLocaleString('zh-CN')})`);
      });
      console.log('');
